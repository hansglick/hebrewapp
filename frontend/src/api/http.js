...
async function throwWithDetail(res, path) {
  ...
  try {
    ...
    if (typeof body?.detail === "string") {
      ...
      // Erreur de validation FastAPI (422, ex: en-tête X-Pseudo/X-Pin
      // manquant) : `detail` est un tableau d'objets {loc, msg, type}, pas
      // une chaîne — passé tel quel à `new Error()` plus bas, il serait
      // silencieusement coercé en "[object Object]" (bug rapporté par le
      // user lors d'une évaluation orale groupée).
