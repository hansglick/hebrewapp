...
# Même seuil que app.routers.jdr/app.routers.revision : sous cette taille, un
# blip de bruit/souffle déclencherait un appel Whisper pour rien.
MIN_TURN_BYTES = 9600
...
def get_concept(code: str):
    ...
    item = concept.get_concept_item(code)
    if concept.get_concept_instruction(code) is None or item is None:
        raise HTTPException(404, f"Aucun concept pour la leçon {code!r}")
    return {
        "lesson_code": code,
        "concept": item["concept"],
        "image_url": concept.concept_image_url(code),
    }
...
async def concept_ws(websocket: WebSocket, code: str, pseudo: str, pin: str):
    ...
    # ici par la query string (cf. api/concept.js::conceptWebSocketUrl).
    if get_user_id(pseudo, pin) is None:
        await websocket.send_json({"type": "error", "message": "Identité inconnue"})
        await websocket.close()
        return

    instruction = concept.get_concept_instruction(code)
    if instruction is None:
        await websocket.send_json({"type": "error", "message": f"Aucun concept pour la leçon {code!r}"})
        await websocket.close()
        return
    # {pseudo} reste littéral dans item_concept_revision.json (généré hors
    # ligne, sans connaître l'utilisateur) — cf. CONCEPT_SYSTEM_INSTRUCTION_TEMPLATE.
    ...
    try:
        async with concept.live_client().aio.live.connect(
            model=concept.LIVE_MODEL, config=concept.build_live_config(instruction)
        ) as session:
            await session.send_client_content(
                turns=types.Content(role="user", parts=[types.Part.from_text(text=concept.AMORCE)]),
                turn_complete=True,
            )
