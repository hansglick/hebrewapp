...
def get_examens_summary():
    conn = get_connection()
    try:
        current_level = _current_level(conn)
        rows = conn.execute(
...
def get_examen_copie(attempt_id: int):
    conn = get_connection()
    try:
        row = conn.execute(
            """
    ...
    try:
        ...
        if row is None:
            hard_row = conn.execute(
                "SELECT 1 FROM hard_exam_sessions WHERE user_id = ?", (DEFAULT_USER_ID,)
            ).fetchone()
    finally:
        conn.close()
...
def get_examen(code: str, god_mode: bool = False):
    conn = get_connection()
    try:
        ...
        try:
            questions, answers, created_at, paused_seconds = get_or_create_session(
                conn, code, "ecrit", code, build_exam, god_mode
            )
...
def get_examen_oral(code: str, god_mode: bool = False):
    conn = get_connection()
    try:
        try:
            questions, answers, _created_at, _paused_seconds = get_or_create_session(
                conn, code, "oral", code, build_oral_exam, god_mode
            )
        except DailyCapReached:
            raise HTTPException(429, "Plus de tentative disponible aujourd'hui pour ce format")
    finally:
...
def get_session_exists(code: str):
...
def get_examen_status(code: str):
    ...
    try:
        ...
        last_passed_ecrit = _last_passed_metrics(conn, code, "ecrit")
        last_passed_oral = _last_passed_metrics(conn, code, "oral")
        gate = entry_gate_code(code)
        gate_passed = gate is None or {"ecrit", "oral"} <= _passed_exam_types(conn, gate)
        status = {
            "ecrit_passed": "ecrit" in passed_types,
            "oral_passed": "oral" in passed_types,
            "entry_gate": gate,
            "entry_gate_passed": gate_passed,
            "attempts_today_ecrit": _attempts_today(conn, code, "ecrit"),
            "attempts_today_oral": _attempts_today(conn, code, "oral"),
            "last_score_ecrit": _last_score(conn, code, "ecrit"),
            "last_score_oral": _last_score(conn, code, "oral"),
            ...
            "full_history_ecrit": _full_history(conn, code, "ecrit"),
            "full_history_oral": _full_history(conn, code, "oral"),
            "fallback_level": _highest_fully_passed_code_excluding(conn, code),
            "points_a_gagner_ecrit": wallet.points_a_gagner(conn, code, "ecrit"),
            "points_a_gagner_oral": wallet.points_a_gagner(conn, code, "oral"),
            **wallet.recompense_info(conn, code),
...
def _last_score(conn, code: str, exam_type: str):
    row = conn.execute(
        """
...
def _last_passed_metrics(conn, code: str, exam_type: str):
...
def _full_history(conn, code: str, exam_type: str, limit: int = 5) -> list:
...
def answer_examen(code: str, payload: ExamAnswerRequest):
    ...
    try:
        try:
            result = record_answer(
                conn, code, payload.exam_type, payload.question_index, payload.answer, payload.pause_seconds
            )
...
def abandon_examen(code: str, payload: ExamAbandonRequest):
    if payload.exam_type not in ("ecrit", "oral"):
        raise HTTPException(400, "exam_type invalide")

    conn = get_connection()
    try:
        result = abandon_session(conn, code, payload.exam_type)
    finally:
        conn.close()
